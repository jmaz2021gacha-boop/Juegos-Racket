...
;; ============================================================
;; ESCALA VISUAL DE LA VENTANA
;; ============================================================
;; No cambia las coordenadas internas del juego.
;; Solo reduce el tamaño de lo que se muestra en pantalla.
;; Valores recomendados:
;; 1.00 = tamaño original
;; 0.80 = más pequeño
;; 0.75 = recomendado
;; 0.70 = más pequeño todavía
(define ESCALA-VENTANA 0.85)
...
;; Botón de reinicio dentro del nivel
(define BOTON-REINICIAR-X 1010)
(define BOTON-REINICIAR-Y 45)
(define BOTON-REINICIAR-W 120)
(define BOTON-REINICIAR-H 36)
...
;; RENDER PRINCIPAL
;; ============================================================
(define (imagen-boton-reiniciar)
  (overlay
   (text "Reiniciar" 16 "white")
   (rectangle BOTON-REINICIAR-W BOTON-REINICIAR-H "outline" "yellow")
   (rectangle BOTON-REINICIAR-W BOTON-REINICIAR-H "solid" "black")))

(define (dibujar-boton-reiniciar escena)
  (place-image
   (imagen-boton-reiniciar)
   BOTON-REINICIAR-X
   BOTON-REINICIAR-Y
   escena))
...
;; DIBUJO ESCALADO
;; ============================================================
;; Dibuja todo el mundo original, pero lo muestra más pequeño.
;; Las coordenadas internas siguen siendo WIDTH x HEIGHT.
(define (dibujarmundo-escalado est)
  (scale ESCALA-VENTANA
         (dibujarmundo est)))
...
(define (manejar-mouse est x y evento)
  (if (not (string=? evento "button-down"))
      ...
      (cond
        ...
        [(string=? (estado-modo est) "jugando")
         (if (en-rect-centro? x y
                              BOTON-REINICIAR-X BOTON-REINICIAR-Y
                              BOTON-REINICIAR-W BOTON-REINICIAR-H)
             (crear-estado-nivel (estado-nivel-id est))
             est)]
...
;; MOUSE ESCALADO
;; ============================================================
;; Como la imagen se muestra reducida, el clic visible debe
;; convertirse a las coordenadas internas originales.
(define (manejar-mouse-escalado est x y evento)
  (manejar-mouse est
                 (/ x ESCALA-VENTANA)
                 (/ y ESCALA-VENTANA)
                 evento))
...
(big-bang estado-inicial
  (to-draw    dibujarmundo-escalado)
  (on-key     mov_ambos)
  (on-tick    actualizar_ambos 1/30)
  (on-release parar_ambos)
  (on-mouse   manejar-mouse-escalado))